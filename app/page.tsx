...
function routeFromHash(): RouteKey {
  ...
  if (route === "nanobot" || route === "dsh" || route === "compare") return route;
  return "home";
}

export default function Home() {
  const [route, setRoute] = useState<RouteKey>("home");
  const [lessonId, setLessonId] = useState<Record<ProjectKey, string>>({ nanobot: "N01", dsh: "D01" });
  ...
  useEffect(() => {
    const sync = () => {
      ...
      if ((next === "nanobot" || next === "dsh") && bits[1]) {
  ...
  return (
    <header className="topbar">
      ...
      <nav className={menuOpen ? "topnav open" : "topnav"} aria-label="主导航">
        ...
        <a className="github-button" href="https://github.com/HKUDS/nanobot" target="_blank" rel="noreferrer" aria-label="查看 nanobot GitHub">GH</a>
    ...
    <main className="landing">
      <section className="hero-grid">
        <div className="hero-copy">
          ...
          <p>两条源码路线，一套统一问题：消息怎么进来、上下文怎么组装、模型如何调用工具、状态如何留下、系统如何长成产品。</p>
          <div className="hero-actions">
            <button className="primary-action" onClick={() => navigate("nanobot")}>从 nanobot 开始 <span>→</span></button>
            <button className="text-action" onClick={() => navigate("compare")}>先看两者差异</button>
          </div>
          <div className="hero-stats">
            <div><strong>16</strong><span>节架构课</span></div>
            <div><strong>2</strong><span>源码路线</span></div>
      ...
      <section className="route-section">
        <div className="section-heading">
          <span>选择一条路线</span>
          <p>两个项目都从 Agent Loop 出发，但它们选择了完全不同的扩展方式。</p>
        </div>
  ...
  return (
    <button className={`route-card ${project}`} onClick={() => navigate(project)}>
      <div className="route-card-top">
        <span className="route-index">{project === "nanobot" ? "ROUTE A" : "ROUTE B"}</span>
        <span className="route-lang">{data.language}</span>
      </div>
      <div className="route-icon">{project === "nanobot" ? "nano / bot" : "dsh / core"}</div>
      <h2>{data.title}</h2>
      <p>{data.intro}</p>
...
function Compare({ navigate }: { navigate: (route: RouteKey) => void }) {
  const rows = [
    ["核心组织方式", "分层 Python 模块 + 注册表", "Cordis 插件树 + ctx"],
    ["消息入口", "异步 MessageBus", "Agent inbox + typed events"],
    ["Agent 执行", "AgentLoop / AgentRunner 分工", "agent-loop 插件 + turn/step"],
    ["状态真源", "Session 历史 + 长期 Memory", "append-only SessionEvent"],
    ["扩展工具", "ToolRegistry / plugin entry points", "ctx.tools + waterfall 管线"],
    ["模型替换", "Provider 接口与 registry", "ctx.llm seam provider"],
    ["产品组装", "Gateway 作为 composition root", "Profile / Bundle / patch"],
    ["最强教学价值", "清晰、直接、容易追调用链", "高度可组合、边界极致显式"],
  ];
  return (
    <main className="compare-page">
      <div className="compare-hero">
        <div className="eyebrow"><span /> SIDE BY SIDE · 横向对照</div>
        <h1>同一个 Agent 问题，<br /><em>两种系统答案。</em></h1>
        <p>nanobot 选择清晰的应用分层；DSH 选择彻底的插件组合。它们不是强弱关系，而是不同的复杂度预算。</p>
      </div>
      <div className="comparison-table">
        <div className="comparison-head"><span>架构问题</span><button onClick={() => navigate("nanobot")}><b>nanobot</b><small>Readable Application</small></button><button onClick={() => navigate("dsh")}><b>DSH</b><small>Composable Harness</small></button></div>
        {rows.map((row, index) => <div className="comparison-row" key={row[0]}><span><small>{String(index + 1).padStart(2, "0")}</small>{row[0]}</span><p>{row[1]}</p><p>{row[2]}</p></div>)}
      </div>
      ...
      <section className="final-thesis"><small>THE THESIS</small><blockquote>nanobot 教你把 Agent <em>分清楚</em>；DSH 教你把 Agent <em>换得掉</em>。</blockquote></section>
    </main>
  );
}
